...
void testApp::setup(){
    ...
    audioBuffer=new AudioBuffer(&audioInput);
    ...
    */
    ...
    //create gui controls
    gui.init(830,40,220,120);
    gui.addSpinSlider("multix delay offset",&(multix->delayOffset),0,10000);
    gui.addSpinSlider("multix speed offset",&(multix->speedOffset),-2,2,.001);
    gui.addSpinSlider("audio density",&(audioHeader->density),0,50);
    gui.addSpinSlider("audio attack",&(envelope.attackPct),0,1);
    gui.addSpinSlider("audio decay",&(envelope.decayPct),0,1);
    gui.addSpinSlider("multix video in",&(multix->in),0,1);
    gui.addSpinSlider("multix video out",&(multix->out),0,1);
    gui.addToggle("multix loop",&(multix->loopMode));
    gui.addSpinSlider("multix scale",&(multix->scale),0,10);

    gui.addSpinSlider("multix rotZ offset",&(multix->rotZOffset),0,360,.01);
    gui.addSpinSlider("multix rotY offset",&(multix->rotYOffset),0,360,.01);
    gui.addSpinSlider("multix num headers",&(multix->numHeaders),1,300);
    gui.addSpinSlider("background",&(background),0,255);

    gui.addSpinSlider("multix alpha",&(multix->alpha),0,255);

    gui.addSpinSlider("multix z",&(multix->z),-5000,100);
    gui.addSpinSlider("multix anchor z",&(multix->anchorZ),-5000,+5000);

    gui.addSpinSlider("multix xOffset",&(multix->xOffset),0,1000);
    gui.addSpinSlider("multix yOffset",&(multix->yOffset),0,1000);
    gui.addSpinSlider("multix xPos",&(multix->x),-10000,10280);
    gui.addSpinSlider("multix yPos",&(multix->y),-10280,10280);


    gui.addToggle("multix blend 0-min 1-max",&(multix->minmaxBlend));
    gui.addToggle("multix depth test",&(multix->depthTest));

    gui.addSpinSlider("multix yRotSpeed",&(multix->yRotSpeed),-100,100,.01);
